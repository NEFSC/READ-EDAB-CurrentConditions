...
##dhdc_7-2024-fall spring data.csv
## SHOULD NOT BE COMBINED INTO A SINGLE ANNUAL VALUE... RUN FOR BOTH

####################################################################
# 1
# set working directory
# setwd(choose.dir(default=getwd()))
setwd("d:/1_one/1_analyses_ne_shelf/along shelf pos")
wd=getwd()
####################################################################
####################################################################
# 2
# name of survey data file
#surfilename="survdat_catch_2024_4.rds"
#surfilename="Survdat_2023_07.Rda"
surfilename="survdat_2024_10.rds"
####################################################################
####################################################################
# 3
# select season of the data and select output file
# SPRING 48 species
selseaon="SPRING"
outfile="dhdc_7_2024_fallASS_sprDATA.csv"

# fall 48 species
#selseaon="FALL"
#outfile="dhdc_1_2024_sprASS_fallDATA.csv"

# SPRING ALL 175
#selseaon="SPRING"
#outfile="dhdc_1_2023_fallASS_sprDATA_ALL.csv"

# FALL ALL 175
#selseaon="FALL"
#outfile="dhdc_1_2023_sprASS_fallDATA_ALL.csv"
####################################################################
####################################################################
# 5
...
sps=read.csv(file="sps_spring_fall_combined.csv", header = TRUE)
numsps=nrow(sps)
####################################################################
####################################################################
# 6
# species to keep in analysis
# for ALL
#spptokeep=sps$SVSPP
...
# the 48 best model distribution speccies
...
# get survey datafile  Survdat.RData

#splitname = strsplit(surfilename,"[.]")
#if(unlist(splitname)[2]=="rds"){
#  survdat <- readRDS(surfilename)
#}
#if(unlist(splitname)[2]=="rda"){
#  load(file=surfilename)
#}
survdatData <- readRDS(surfilename)
survdat = survdatData$survdat
